...
namespace NethereumSample
{
    ...
    public class MyWallet
    {
        ...
        public MyWallet(Wordlist wordList, WordCount wordCount, string seedPassword = null, string path = DEFAULT_PATH,
            IRandom random = null) : this(path, random)
        {
            InitialiseSeed(wordList, wordCount, seedPassword);
        }

        public MyWallet(string words, string seedPassword, string path = DEFAULT_PATH, IRandom random = null) : this(path,
            random)
        {
            InitialiseSeed(words, seedPassword);
        }

        public MyWallet(byte[] seed, string path = DEFAULT_PATH, IRandom random = null) : this(path, random)
        {
            Seed = seed.ToHex();
        }

        private MyWallet(string path = DEFAULT_PATH, IRandom random = null)
        {
            Path = path;
